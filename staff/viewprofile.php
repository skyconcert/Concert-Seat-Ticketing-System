...
$colname_updateUserRecset = "-1";
if (isset($_GET['aid'])) {
  $colname_updateUserRecset = $_GET['aid'];
}
...
<div id="wrapper">
	...
	<table border="1" align="center" width="1000" id="admin" onSubmit="return editFunction()">
		<tr>
		...
		<th style="text-align:center; background-color:#FFB6C1"><font color="white">Update</font></th>
		</tr>
<?php
	include "dbcon.php";
	
	$aid=$_SESSION['admin'];
	$sql = "SELECT aid, name, password, phone, userlevel FROM admin WHERE aid='$aid'";
	$count=1;
	$result = mysql_query($sql);
	while($row = mysql_fetch_assoc($result))
	{
        Print '<tr align="center" color="#fff2e6">
                  <td height="55">'.$row['aid'].'</td>
                  <td>'.$row['name'].'</td>
				  <td class="hidetext">'.$row['password'].'</td>
				  <td>'.$row['phone'].'</td>
				  <td>'.$row['userlevel'].'</td>
                  <td><a href="editstaff.php?aid='.$row['aid'].'"><img src="images/edit.jpg" width="23" height="23"/></a></td>
				</tr>';	
				$count=$count+1;
	}
$totaladmin=$count-1;
